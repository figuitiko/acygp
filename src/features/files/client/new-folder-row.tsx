"use client";

import { useState } from "react";

import { Check, Plus, X } from "lucide-react";

type NewFolderRowProps = {
  action: (formData: FormData) => void | Promise<void>;
  parentId?: string | null;
  placeholder?: string;
  triggerLabel?: string;
  autoOpen?: boolean;
  onCancel?: () => void;
};

export function NewFolderRow({
  action,
  parentId = null,
  placeholder = "Nombre de la carpeta",
  triggerLabel = "Nueva carpeta",
  autoOpen = false,
  onCancel,
}: NewFolderRowProps) {
  const [isCreating, setIsCreating] = useState(autoOpen);

  function close() {
    setIsCreating(false);
    onCancel?.();
  }

  if (isCreating) {
    return (
      <form
        action={action}
        className="flex items-center gap-1 rounded-lg border-2 border-main bg-white py-1 pl-2 pr-1"
      >
        <input type="hidden" name="parentId" value={parentId ?? ""} />
        <input
          name="name"
          autoFocus
          required
          placeholder={placeholder}
          className="w-full min-w-0 rounded-md px-1 py-1 text-sm font-semibold text-slate-900 outline-none"
        />
        <button
          type="submit"
          aria-label="Crear carpeta"
          className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-emerald-700 transition hover:scale-110 hover:bg-emerald-600 hover:text-white"
        >
          <Check size={13} />
        </button>
        <button
          type="button"
          onClick={close}
          aria-label="Cancelar"
          className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-slate-500 transition hover:scale-110 hover:bg-slate-600 hover:text-white"
        >
          <X size={13} />
        </button>
      </form>
    );
  }

  return (
    <button
      type="button"
      onClick={() => setIsCreating(true)}
      className="inline-flex items-center gap-1 rounded-lg border border-dashed border-slate-300 px-3 py-1.5 text-sm font-bold text-slate-500 transition hover:border-main hover:text-main"
    >
      <Plus size={14} />
      {triggerLabel}
    </button>
  );
}
